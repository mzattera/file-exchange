 ...
 */
public class CriticModule extends Agent {
	...
	public static final String PROMPT_TEMPLATE = "# Identity\n\n" //
			+ "You are a critic agent; your task is to monitor how an executor agent tries to execute user's commands and provide suggestion to improve execution.\n" //
			+ "The specific user's command the executor is trying to execute is provided in the below <user_command> tag.\n" //
			+ "\n<user_command>\n{{command}}\n</user_command>\n\n" //
			+ "You will be provided by the user with a potentially empty list of execution steps, in <steps> tag, that have been performed by the executor in its attempt to execute the user's command. The format of these steps is provided as a JSON schema in <step_format> tag below. In these steps, the executor agent is identified with actor==\"{{executor_id}}\", other agents and tools are identified as other actors.\n"
	...
	public static final String REVIEW_TOOL_CALL_TEMPLATE = PROMPT_TEMPLATE //
			+ "\n# Instructions\n\n" //
			+ "  * If execution steps contain evidence that the executor entered a loop calling same tool repeatedly with same parameters, provide a suggestion to strictly call another tool to execute next step.\n"
			...
			+ "  * **STRICTLY**, in all other case, or if you do not have any relevant suggestion about how to change the executor behavior, just output \"CONTINUE\"."
			+ "  * **STRICTLY** your output is either a suggestion or \"CONTINUE\", do not add any comment if you emit \"CONTINUE\" and do not use \"CONTINUE\" if you have a suggestion.";

	public static final String REVIEW_CONCLUSIONS_TEMPLATE = PROMPT_TEMPLATE //
			+ "\n# Instructions\n\n" //
			+ "  * The only evidence that the executor agent called a tool or performed a specific task is in the \"action\" field of any execution step; " //
			...
			+ "  * IF AND ONLY IF last execution step has status=\"ERROR\", it means the executor is aborting execution because it encountered an error it cannot resolve. " //
			+ "In this case, examine carefully the execution steps, try to determine what went wrong; if you are able to identify a possible error source and a remediation approach, " //
			+ "then output a suggestion detailing how the error can be avoided. For example, if the executor is trying to use the wrong tool to perform a task, suggest the proper tool to use. " //
			+ "If you cannot find a remediation, or if \"observation\" field for most recent execution steps suggest repeating errors, just output \"CONTINUE\".\n"
			+ "  * IF AND ONLY IF last execution step has status=\"COMPLETED\", check it carefully for indication in \"observation\" or \"thought\" fields that the executor has some more steps to perform to complete the user's command. If this is the case, THEN output a suggestion to strictly proceed with execution of these steps; suggest to call a specific tool if you see fit.\n"	
			+ "  * IF executor agent mentions it needs to proceed with next steps, or lists tasks it still needs to perform, but is not performing any action, " //
			+ "THEN output a suggestion to strictly proceed with execution of these steps; suggest to call a specific tool if you see fit.\n"	
			+ "  * IF executor agent keeps outputting same execution steps, " //
			+ "THEN output a suggestion to strongly and strictly stop to output same steps; suggest to call a specific tool if you see fit.\n"			
			+ "  * **STRICTLY**, in all other cases, or if you do not have any relevant suggestion about how to change the executor behavior, just output \"CONTINUE\"."
			+ "  * **STRICTLY** always focus on current command execution, never output suggestions about how to fix errors next time the executor agent will execute the command.\n" //
			+ "  * **STRICTLY** your output is either a suggestion or \"CONTINUE\", do not add any comment if you emit \"CONTINUE\".\n"
			+ "\n# Examples\n\n" //
			+ "Input:\n\n" //
			...
			+ "</steps>\n" //
			+ "\nCorrect Output:\n\n" //
			+ "There is no evidence you sent the email as needed, no action was performed; call <tool ID here> tool to send the email.\n" //
			+ "\nIncorrect Output:\n\n" //
			+ "CONTINUE\n" //
			+ "\n---\n\n" //
			+ "Input:\n\n" //
			...
			+ "  \"status\" : \"COMPLETED\",\n" //
			+ "  \"actor\" : <Executor ID here>,\n" //
			+ "  \"thought\" : \"The only remaining step is to close the task. I will now call the \"closeTask\" tool to close the task.\",\n" //
			...
			+ "</steps>\n" //
			+ "\nCorrect Output:\n\n" //
			+ "There is no evidence you closed the task as needed, no action was performed; call the \"closeTask\" tool to close the task.\n" //
			+ "\nIncorrect Output:\n\n" //
			+ "CONTINUE\n" //
			+ "\n---\n\n" //
			+ "Input:\n\n" //
